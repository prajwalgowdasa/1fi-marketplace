...
export function ProductGallery({ images }: { images: readonly ProductImage[] }) {
  ...
  return (
    <section aria-label="Product images">
      ...
      {images.length > 1 ? (
        <div className="mt-3 flex gap-3" role="list">
          {images.map((image, index) => (
            <button aria-label={`View ${image.alt}`} aria-pressed={selectedIndex === index} className="min-h-11 min-w-11 overflow-hidden rounded-xl border-2 border-transparent aria-[pressed=true]:border-[var(--brand-500)]" key={image.src} onClick={() => setSelectedIndex(index)} type="button">
